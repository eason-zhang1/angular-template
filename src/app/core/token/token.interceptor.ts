...
export class TokenInterceptor implements HttpInterceptor {
  ...
  private goLogin() {
    ...
    this.injector.get(Router).navigate(['/user/login']);
  }
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpSentEvent | HttpHeaderResponse | HttpProgressEvent | HttpResponse<any> | HttpUserEvent<any>> {
    ...
    if (req.url.includes('/api/mgt')) {
      ...
      header = req.headers.set('x-auth-token', authData.access_token);
    }
    if (header != null) {
    ...
    return next.handle(newReq).mergeMap((event: any) => {
        ...
        // 以错误的形式结束本次请求
        return Observable.throw(JSON.stringify(res));
      });
  }
}
